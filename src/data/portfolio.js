...
export const PROJECTS = [
  ...
  {
    name: "threadroom",
    tagline: "Several AI Models Working on One Document",
    stack: [
      "Python",
    ...
    meta: "Solo Developer · 2026 · Live · Local App",
    link: "https://threadroom-site.vercel.app",
    bullets: [
      "Built a local app that seats several AI models in a shared room and has them argue their way to a single deliverable - they open the conversation themselves, read each other's actual files rather than descriptions of them, revise a section at a time, and hand back a document rather than a transcript.",
      "Ran the whole thing on the user's own machine with nothing hosted, nothing uploaded, and no API keys to buy: seats are driven by the AI vendor's own CLI, so they sign in with a subscription the user already pays for.",
      "Launched every seat with all tools switched off and made text the only tool channel - a seat emits a sentinel block and the host process executes it. An agent holding real tools stalls on a permission prompt, and a stalled room needs a human to click Approve before two models can finish a sentence; a room whose only channel is text cannot stall.",
      "Designed a write protocol that survives simultaneous authors: every write is checked against the revision the seat was actually shown and refused if stale, section-scoped edits rebase and merge when nobody touched that section, and a seat that loses a genuine collision is owed the next write so a fast writer cannot permanently lock out a slower one.",
      "Bounded rooms by construction with a per-seat turn allowance, an hourly ceiling, and an idle detector that hands back rather than running up a bill - written after an earlier design spent $100 in thirty minutes talking to itself.",
      "Wrote 58 passing test suites with no pytest and no assertion library: every check is a plain sentence about behavior, each cites the failure it was written for with the date it was measured, and every suite ends with an explicit list of what it does not prove.",
      "Shipped installation as a single curl command that installs the CLI if missing, unpacks the app, and opens the panel, leaving existing rooms untouched on reinstall.",
    ],
  },
  {
    name: "Movie Geo",
    tagline: "Daily Movie-Trivia Game on a 3D Globe",
    stack: [
      "JavaScript",
      "D3-geo",
      "TopoJSON",
    ...
    meta: "Solo Developer · 2026 · Live at movie-geo.com",
    link: "https://movie-geo.com",
    bullets: [
      "Built a daily geography game for film lovers: a clue about where a scene was shot, where a story is set, or where an awards ceremony is held, answered by spinning a 3D globe and tapping the country or U.S. state. Five questions a day free forever, with an endless mode and a 30-day archive behind a subscription.",
      "Rendered the globe as a D3 orthographic projection on Canvas 2D rather than WebGL - a deliberate constraint that avoids a shader pipeline and its cold-start cost on a mid-range phone.",
      "Solved re-projection cost with two-tier resolution: coarse 110m TopoJSON while the globe is in motion, swapped for detailed 50m geometry the moment it comes to rest, so the transition is invisible in motion and the detail is there when aiming.",
      "Made hit testing forgiving with a spiral probe - a tap that misses samples outward in a widening spiral until it finds land, so a near-miss on a border or a small country resolves to the country the player obviously meant instead of the ocean.",
      "Shipped the entire client as one self-contained HTML document with libraries, map data, and the 1,443-question bank inlined at build time: one request, no waterfall, no CDN dependency, works offline once loaded.",
      "Wrote a build-time giveaway detector that scans each clue for region-revealing tells and decides whether the film title can safely be shown as context - 515 questions display their title, the rest stay blind - then fact-checked the bank and rewrote roughly a dozen wrong answers.",
      "Carried subscription state in a stateless HMAC-signed token minted from the Stripe customer id, with Stripe as the only source of truth, and designed the failure mode so a failed status check keeps the last-known state rather than locking a paying subscriber out.",
      "Enforced a layout budget in CI: a Playwright suite plays real rounds across iPhone 13, iPhone SE, landscape, and desktop, and fails the build if the HUD covers more than 46% of a phone screen.",
    ],
  },
  {
    name: "Family Arcade",
